...
def postprocessdata():
	...
	data = json.load(f)
	f.close()
	nd = {k:v for k,v in data.items() if v['WeatherZone'] == ''}
...
def ClusterByWeatherZones(CombinedDataERCOT):
	...
	data = {z: [[v['Coordinates'][0], v['Coordinates'][1], v['load'], 'None', k] for k,v in CombinedDataERCOT.items() if z == v['WeatherZone']] for z in zones if z != 'Others'}
...
if __name__ == '__main__':
	...
	LoadDataList = [[v['Coordinates'][0], v['Coordinates'][1], v['load'], 'None', k] for k,v in CombinedLoadDataERCOT.items() if v['load'] != 0 if v['WeatherZone'] != 'Others'] # Passing k to the list, however it is not used currently in Cluster data structure
	from data2html import data2htmlmain
	pointslistGen = data2htmlmain()
	...
	clusterno = [1800-100*i for i in range(18)] + [50, 30, 20, 10, 8, 5] # [8] - M1 # range(18) 1915 - Preprocessed (Removed overlapping gen data in Clustering Algorithm i.e., retained only plants)

	print('Clusterno:', clusterno)

	metric='euclidean'
	#cdata = case_2(CombinedLoadGenList, metric, clusterno)
	ZIPCodesByCluster = case_2(CombinedLoadGenList, metric, clusterno)
