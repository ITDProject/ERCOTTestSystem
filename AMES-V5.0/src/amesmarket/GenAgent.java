 ...
 *
 * GenData //ID	atBus	SCost	A	B	CapMin	CapMax	NS initMoney 1	1	5	15	0.10	0	100	1000
 *
 */
...
public class GenAgent implements Drawable, JReLMAgent {
    ...
    private static final int NSeg = 7;
    private static final int INIT_MONEY = 8;
    ...
    public GenAgent(double[] genData, VREParameters learningParams,
        ...
        trueSupplyOffer = new double[NSeg - A + 1];
        for (int i = 0; i < NSeg - A + 1; i++) {
    ...
    if(Double.isNaN(policy.getProbability(id))){
         ...
         */
        if (iSCostExcludedFlag == 0)// profit
        {
            learner.update(new Double(dailyProfit));
        }

        if (iSCostExcludedFlag == 1)// net earnings
        {
...
//    }
    ...
    public void setColdStartHours(int n) {
        this.coldStartHours = n;
    }
    ...
    public int getColdStartUpHours() {
        return this.coldStartHours;
    }
...
//        }
        ...
        // numHourSame++; //  - commented it as the first hour has already been counted 

        if (lastHourState == 1) { //Generator has been on.
            if (numHourSame == 24){
                setUnitOnT0State(numHourSame+UnitOnT0State);
                return (numHourSame+UnitOnT0State);
            }
            else{
...
//        }
    ...
    public void addDailyCosts(int interval, double[] startCosts,
            double[] productionCosts, double[] shutdownCosts) {
        productionCostsByDay.put(interval, Arrays.copyOf(productionCosts, productionCosts.length));
        startupCostsByDay.put(interval, Arrays.copyOf(startCosts, startCosts.length));
        shutdownCostsByDay.put(interval, Arrays.copyOf(shutdownCosts, shutdownCosts.length));
    }
