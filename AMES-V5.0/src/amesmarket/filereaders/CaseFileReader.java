        ...
        for (int i = 0; i < iGenNumber; i++) {
            ...
            try {
                testConf.genData[i] = new GenData(
                    ...
                    Support.parseInt(lineElems[8]),//NS
                    Support.parseDouble(lineElems[9])//initMoney
                );
...
//                            }
    ...
    private void setDefaultReserveZoneData(CaseFileData testConf) {
       ...
       testConf.putZoneData("Zone1",
                        BusArray,//Buses
                        0.0,//ReserveDownZonalPercent
                        0.0 //ReserveUpZonalPercent
                );
    }
    ...
    private void parseGenCoCosts(CaseFileData testConf) throws BadDataFileFormatException {
        ...
        for(String noLoad : zoneNoLoads) {
            ...
            try{
                ...
                testConf.addColdStartHours(p[0], Support.parseInt(p[2]));
                testConf.addColdStartUpCost(p[0], Support.parseDouble(p[3]));
                testConf.addHotStartUpCost(p[0], Support.parseDouble(p[4]));
                testConf.addShutDownCost(p[0], Support.parseDouble(p[5]));
            } catch(Exception e) {
                throw new BadDataFileFormatException(inputReader.sourceFile,
