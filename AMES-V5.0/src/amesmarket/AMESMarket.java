...
//maybe this is related with input output method
    ...
    public void buildModel() {
        ...
        //System.out.println("Printing all parameters before simulation is started in AMESMarket.java \n");
...
//        }
    ...
    private void printStructuralParameters() {
        ...
        for (int i = 0; i < genData.length; i++) {
            strTemp = String.format("\t%1$15d\t%2$15d\t%3$15.4f\t%4$15.4f\t%5$15.4f\t%6$15.4f\t%7$15.4f\t%8$15.4f",
                    ...
                    genData[i][4], genData[i][5], genData[i][6], genData[i][7], (int) genData[i][8]);
            System.out.println(strTemp);
        }
    ...
    private void addGenAgents(java.util.Random rn) {
        ...
        for (int i = 0; i < numGenAgents; i++) {
            ...
            if (testcaseConfig.hasColdStartHours(gcName)) {
                gen.setColdStartHours(testcaseConfig.getColdStartHoursForGen(gen.getID()));
            }
...
//                if (min % min_len == 0) {
//                    System.out.println("\n\nDay: " + day + " Hour: " + hour + " Interval: " + interval + " Minute: " + min);
//                }

                System.out.println("\nMarketOperation for Day: " + day + " Hour: " + hour + " Interval: " + interval + " Minute: " + min);
...
//                time_next = (day - 1) * (day_len) + (hour+1) * (hour_len) + min * min_len;
                ...
                if (hour == 13 && min == 0) {
                    if (FNCSActive) {
                        JNIfncs.publish("DailyLMP", getStrings(iso.getDAMLMP(), 2)); //node 3
                    }
                    System.out.println("");
                    for (int iter = 0; iter < NumNodes; iter++) {
                        System.out.println("DAM LMP published values: " + getStrings(iso.getDAMLMP(), iter));
                    }
                }
