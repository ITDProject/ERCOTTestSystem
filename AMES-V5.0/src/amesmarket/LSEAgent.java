...
//method
    ...
    public double[] submitDAMLoadForecast(int day, int lse, int psLse, boolean FNCSActive) {

        double[] temp = new double[24];
        // Receives load forecast through fncs
        //System.out.println("LSE"+(lse+1)+ " submitDAMLoadForecast Method");
        if (day > 1) { // previously day > 2 // && lse == (psLse - 1)
            if (FNCSActive) {
    ...
    public double[][] submitDemandBid(int day, int lse, int psLse, boolean FNCSActive) {
        double[][] reportDemandBid = new double[24][3];
