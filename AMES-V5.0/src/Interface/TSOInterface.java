 ...
 *
 * Need to run at least one day Day-ahead market before the next day real-time
 * market operation
 *
 * hour == 2, collect bids from demand and generators for next DA hour == 12,
 * run SCUC to clear the DA market
 *
 * For other hours and minutes, run RT market operation, usually at the time
 * internal of 5 minutes
 *
 *
 * @author huan289
 *
...
//        mainFrameWindow.setVisible(true);
        ...
        return mainFrameWindow.getbLoadCase();
    }
    ...
    public boolean runDAMarket(int day) {
        amesMkt.getISO().loadProfileByLSE = amesMkt.getISO().dam.getLoadProfileByLSE();
        amesMkt.getISO().GenProfileByNDG = amesMkt.getISO().dam.getGenProfileByNDG();
    ...
    public double[][] getDALMPs() {
        return amesMkt.getISO().getDAMLMP();
    }
    ...
    /**
     * The original data initialization is in setInitialStructuralParameters() of AMESMarket class. setNodeData(init.getNodeData());
    ...
    public boolean prepareDayAheadSCUCCase(double[][] unRespMW, double[][] respMaxMW, double[][] respC2, double[][] respC1, double[][] resp_deg, double[][] forecast) {
        ...
        /**
         * Note currently DA market does not support responsive loads. The
         * following data not used!!!!
         *
         * double[][] respMaxMW, double[][] respC2, double[][] respC1,
         * double[][] resp_deg data not used!!!!
         */
        double[][] loadProfileLSE = this.amesMkt.getISO().getLoadProfileByLSE();
        double[][] GenProfileByNDG = this.amesMkt.getISO().getGenProfileByNDG();
