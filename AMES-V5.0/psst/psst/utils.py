...
def read_model(model_data):
    ...
    for l in data.splitlines():
        if l.startswith('param HasZonalReserves'):
            zonalData['HasZonalReserves'] = bool(l.split(':=')[1].split(';')[0].replace(' ', ''))
    ...
    for l in data.splitlines():
        ...
        if READ is True:
            ...
            c.gencost.loc[g, "COLD_START_HOURS"] = float(max_g.replace(',', '.')) if cold_start_hours == 0 else cold_start_hours
    ...
    return c, ZonalDataComplete #, NDGData
